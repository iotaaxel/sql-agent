...
class ToolRegistry:
    ...
    def list_tools(self) -> List[Dict[str, str]]:
        ...
        return [
            {"name": name, "description": func.description} for name, func in self.tools.items()
        ]
    ...
    def execute(self, name: str, **kwargs) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(success=False, output=f"Error executing tool: {e!s}")
...
def create_tool_registry(executor, llm_provider, schema_introspector) -> ToolRegistry:
    ...
    def explain_query_plan(query: str) -> ToolResult:
        ...
        if plan:
            return ToolResult(success=True, output=f"Query Execution Plan:\n{plan}", data=plan)
        return ToolResult(success=False, output="Could not generate query plan")
    ...
    def fix_query(query: str, error: str) -> ToolResult:
        ...
        return ToolResult(
            success=True, output=f"Fixed Query:\n```sql\n{fixed_query}\n```", data=fixed_query
        )

    def summarize_results(
        query: str, results: List[Dict[str, Any]], columns: Optional[List[str]] = None
    ) -> ToolResult:
...
"""
        ...
        system_prompt = (
            "You are a data analyst. Summarize SQL query results in a clear, concise manner."
        )

        try:
            summary = llm_provider.generate(prompt=prompt, system_prompt=system_prompt)
            return ToolResult(success=True, output=summary, data=summary)
        except Exception as e:
            logger.error(f"Error summarizing results: {e}")
            return ToolResult(success=False, output=f"Error generating summary: {e!s}")
    ...
    registry.register("fix_query", fix_query, "Fixes SQL syntax errors in a query")

    registry.register(
        "summarize_results", summarize_results, "Summarizes query results using natural language"
    )
