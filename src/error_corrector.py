...
Make sure the corrected query is valid and follows SQL syntax rules."""
        ...
        try:
            corrected_query = self.llm_provider.generate(
                prompt=correction_prompt, system_prompt=system_prompt
            )
    ...
    def _build_correction_prompt(
        self, query: str, error: str, schema: Optional[str], previous_queries: Optional[list]
    ) -> str:
        ...
        if schema:
            prompt_parts.extend(["Database Schema:", schema, ""])
